(() => {
	...
	*/
	...
	function getDocumentHeight() {
		return document.querySelector(".main > article").clientHeight;
	}
	...
	class ReadingPositionIndicator {
		...
		_updateProgressBar(scrollPosition) {
			...
			const percentage = Math.max(Math.round((100 * value) / Math.max(this.scroll.maxHeight, 1)), 0);
	...
	new ReadingPositionIndicator({
		color: "#0C8FE8",
		percentage: {
			show: true,
			displayBeforeScroll: false,
			color: getCookie("theme") == "light" ? "#000" : "#fff",
			opacity: 0.5
		}
	}).init();
})();